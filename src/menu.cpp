...
void Menu::init(unsigned int menuWidth, unsigned int menuHeight,
    ...
    Element elementDataArray[] = {
        ...
        glm::vec4(0.9f, 0.9f, 0.9f, 0.6f), glm::vec4(0.4f, 0.4f, 0.4f, 1.0f),
    ...
    std::string elementTypes[] = {
        ...
        "STONE", "NITROGEN", "CHLORINE", "STEAM", "BIRD", "TIN", "TITANIUM",
    ...
    std::string toolbarTypes[] = { "BOOM!", "GLASSIFY!", "MOVE", "STASIS", "RESIZE", "DELETE", 
    "CLEAR ALL", "CHAOS", "LIGHTNING"};
    for (int i=0; i<TOOLBAR_NUM; i++)   {
        toolbarButtonColor[i].color = glm::vec4(1.0f,0.2f,0.1f,1.0f);
        //Make lightning button yellow
        if(i==8) toolbarButtonColor[i].color=glm::vec4(1.0f, 1.0f, 0.0f, 1.0f);
    ...
    for(int x = 0; x < TOOLBAR_NUM; ++x)    {
        glm::vec2 boxPos;
        //First 7 buttons in one spot. Rest in another
        if(x<7){
            boxPos=glm::vec2(edgeGap + (x * widthGapBetweenBoxes),
            shrunkScrHeight-scrHeightGap);
        }
        else{
            boxPos=glm::vec2(scrWidth,(shrunkScrHeight - ((x-5) * scrHeightGap)));
        }
